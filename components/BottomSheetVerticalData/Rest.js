...
const Rest = () => {
    ...
    const renderRestStopItem = ({ item }) => (
        <View>
            <Text style={[styles.stdTextBottomSheet, { fontWeight: 'bold', marginTop: 15 }]}>{item.name}</Text>
            <View style={[styles.row, { width: 200, marginTop: 5 }]}>
                <Text style={[styles.subHeadingBottomSheet,]}>{item.time}</Text>
                <View style={{ height: 3, width: 3, borderRadius: 50, backgroundColor: 'black' }} />
                <Text style={[styles.subHeadingBottomSheet, { color: 'red' }]}>{item.duration}</Text>
            </View>
        </View>
    );
    ...
    return (
        <View>
            ...
            <Text style={[styles.stdTextBottomSheet, { marginTop: 15, fontWeight: 'bold' }]}>Traveller Experience</Text>
            <FlatList
